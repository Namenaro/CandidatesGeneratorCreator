...
class Derivative1_central(Step):
    ...
    def __init__(self, window_length_int:int = 21, polyorder=2 ):
        self.polyorder = polyorder
        self.window_length_int = window_length_int

    def run(self, signal, left:float, right:float):
        if len(signal) <= 1:
            return []

        if len(signal)>self.window_length_int:
            self.window_length_int = len(signal)
